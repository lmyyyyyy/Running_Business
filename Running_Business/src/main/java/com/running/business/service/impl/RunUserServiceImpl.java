...
public class RunUserServiceImpl implements RunUserService{
	 ...
	 * 检查用户是否存在
	 * @param username 用户账号
	 * @return <p>返回状态码"0"，存在：返回状态码"1"，并返回错误码1001001，错误信息"用户已注册"</p>
	 */
	@Override
	public BaseResult<Object> checkUser(String username) {
	 ...
	 * 添加用户,进行MD5加密
	 * @param user 用户实体
	 * @return <p>添加成功返回状态码"0"</p>
	 */
	@Override
	public BaseResult<Object> addUser(RunUser user) {
	 ...
	 */
	@Override
	public BaseResult<Object> delUser(Integer uid) {
	 ...
	 */
	@Override
	public BaseResult<Object> updateUser(RunUser user) {
		user.setUserPassword(Run_StringUtil.MD5(user.getUserPassword()));
	 ...
	 * 根据用户id获取用户
	 * @param id 用户id
	 * @return 如果没找到该用户，则返回状态码"1"，错误码1001004，错误信息"错误的查询",成功后返回状态码"0",并返回该用户实体
	 */
	@Override
	public BaseResult<Object> getRunUser(Integer id) {
	...
	public BaseResult<Object> login(String username, String password, HttpServletRequest request, HttpServletResponse response) {
		...
		if (!ValidateUtil.isValid(list)) {
			return BaseResult.fail(new ErrorMsg(ResultEnum.PWD_ERROR.getCode(), ResultEnum.PWD_ERROR.getMsg()));
		}
	 ...
	 */
	@Override
	public BaseResult<Object> getAllRunUser() {
	 ...
	 */
	@Override
	public BaseResult<Object> getUserByToken(String token) {
		// 根据token从redis中查询用户信息
		String json = jedisClient.get(REDIS_USER_SESSION_KEY + ":" + token);
		
		//判断是否为空
		if (StringUtils.isBlank(json)) {
			return BaseResult.fail(new ErrorMsg(ResultEnum.SESSION_IS_OUT_TIME.getCode(), ResultEnum.SESSION_IS_OUT_TIME.getMsg()));
		}
		//更新生命周期
		jedisClient.expire(REDIS_USER_SESSION_KEY + ":" + token, SSO_SESSION_EXPIRE);
	 ...
	 */
	@Override
	public BaseResult<Object> logout(String token) {
		// 根据token从redis中查询用户信息
		String json = jedisClient.get(REDIS_USER_SESSION_KEY + ":" + token);
		//判断是否为空
		if (StringUtils.isBlank(json)) {
