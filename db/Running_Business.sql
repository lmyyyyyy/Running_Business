...
CREATE TABLE IF NOT EXISTS `Running_Bussiness`.`run_user_info` (
  ...
  `user_address` VARCHAR(100) NULL COMMENT '当前地址',
  PRIMARY KEY (`uid`))
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `Running_Bussiness`.`run_order` (
  ...
  `status` INT NULL COMMENT '订单状态：0:已下单，1:已支付，2:已接单，3:已到达目的地，4:已完成',
  `add_time` DATETIME NOT NULL COMMENT '创建时间',
...
CREATE TABLE IF NOT EXISTS `Running_Bussiness`.`run_order_pay` (
  ...
  `order_actual_price` DOUBLE NULL COMMENT '实际付款金额',
  `pay_type` INT NOT NULL DEFAULT 0 COMMENT '支付类型：0：余额，1:支付宝，2:微信，3:银行卡 等等',
...
CREATE TABLE IF NOT EXISTS `Running_Bussiness`.`run_admin` (
  ...
  `admin_time` VARCHAR(45) NULL COMMENT '创建时间',
  `update_time` DATETIME NULL COMMENT '更新时间',
