...
CREATE TABLE IF NOT EXISTS `running_business`.`run_order` (
  ...
  `target_time` DATETIME NULL COMMENT '到达目的地时间',
...
-- -----------------------------------------------------
-- Table `running_business`.`heart_beat`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `running_business`.`heart_beat` (
  `id` INT NOT NULL AUTO_INCREMENT COMMENT '心跳id',
  `uid` INT NOT NULL COMMENT '用户/配送员/管理员  id',
  `user_type` INT NOT NULL DEFAULT 0 COMMENT '用户类型：0:用户，1:配送员，2：管理员',
  `ip` VARCHAR(45) NOT NULL COMMENT 'Ip地址',
  `add_time` DATETIME NOT NULL COMMENT '心跳创建时间',
  `update_time` DATETIME NOT NULL COMMENT '最新心跳时间',
  `user_info` VARCHAR(1000) NULL COMMENT '用户登录信息',
  `session_key` VARCHAR(45) NOT NULL COMMENT 'session key',
  PRIMARY KEY (`id`))
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `running_business`.`run_service_log`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `running_business`.`run_service_log` (
  `id` INT NOT NULL AUTO_INCREMENT COMMENT 'service方法日志id',
  ...
  `return_value` TEXT NULL COMMENT '返回值Json格式',
  `time_cost` BIGINT NULL COMMENT '方法耗时',
  ...
  `add_time` DATETIME NOT NULL COMMENT '创建时间',
  `update_time` DATETIME NOT NULL COMMENT '更新时间',
  `parent_id` INT NOT NULL COMMENT '父方法id',
  PRIMARY KEY (`id`))
ENGINE = InnoDB;
CREATE INDEX idx_add_time
  ON run_service_log (add_time);
CREATE INDEX idx_parent_id
  ON run_service_log (parent_id);

-- -----------------------------------------------------
-- Table `running_business`.`run_mapper_log`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `running_business`.`run_mapper_log` (
  `id` INT NOT NULL AUTO_INCREMENT COMMENT 'mapper日志id',
  `serivce_log_id` INT NULL COMMENT 'serivce日志id',
  `method_name` TEXT NOT NULL COMMENT '方法名',
  `method_param` TEXT NULL COMMENT '方法参数',
  `target_table_name` VARCHAR(100) NOT NULL COMMENT '表明',
  `operate_type` TINYINT(3) NOT NULL DEFAULT 0 COMMENT '操作类型',
  `influence_row` INT NULL COMMENT '影响行数',
  `sql_statement` TEXT NOT NULL COMMENT 'sql语句',
  `invoke_status` TINYINT(3) NOT NULL DEFAULT 0 COMMENT '执行状态',
  `error_message` TEXT NULL COMMENT '错误信息',
  `add_time` DATETIME NOT NULL COMMENT '创建时间',
  `update_time` DATETIME NOT NULL COMMENT '更新时间',
...
ENGINE = InnoDB;
CREATE INDEX idx_add_time
  ON run_mapper_log (add_time);
CREATE INDEX idx_service_id
  ON run_mapper_log (serivce_log_id);
